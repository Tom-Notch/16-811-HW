      ...
      \item Consider the function $f (x) = sin x - 0.5$ over the interval $[-\frac{\pi}{2}, \frac{\pi}{2}]$.
            \begin{enumerate}
                  ...
                  \item Graph $f (x)$ over the interval $[-\frac{\pi}{2}, \frac{\pi}{2}]$.
                        ...
                        \begin{figure}[H]
                              \centering
                              \includegraphics[width=0.5\linewidth]{figs/Q1.pdf}
      ...
      return poly, poly_coeff, max_error_abs
                        ...
                        \begin{figure}[H]
                              ...
                              \begin{subfigure}[b]{0.49\linewidth}
                                    \centering
                                    \includegraphics[width=\linewidth]{figs/Q1_c_poly_1.pdf}
                                    \caption{$n = 1$}
                              \end{subfigure}
                              \begin{subfigure}[b]{0.49\linewidth}
                                    \centering
                                    \includegraphics[width=\linewidth]{figs/Q1_c_poly_2.pdf}
                                    \caption{$n = 2$}
                              \end{subfigure} \\
                              \begin{subfigure}[b]{0.49\linewidth}
                                    \centering
                                    \includegraphics[width=\linewidth]{figs/Q1_c_poly_3.pdf}
                                    \caption{$n = 3$, error change does not converge to $< 10^{-5}$ in 1000 iterations}
                              \end{subfigure}
                              \begin{subfigure}[b]{0.49\linewidth}
                                    \centering
                                    \includegraphics[width=\linewidth]{figs/Q1_c_poly_9.pdf}
                                    \caption{$n = 9$}
                              \end{subfigure}
...
def least_square_approximation(f, n: int, interval: list):
    ...
    return poly
                        ...
                        \begin{figure}[H]
                              ...
                              \begin{subfigure}[b]{0.49\linewidth}
                                    \centering
                                    \includegraphics[width=\linewidth]{figs/Q1_d_poly_1.pdf}
                                    \caption{$n = 1$}
                              \end{subfigure}
                              \begin{subfigure}[b]{0.49\linewidth}
                                    \centering
                                    \includegraphics[width=\linewidth]{figs/Q1_d_poly_2.pdf}
                                    \caption{$n = 2$}
                              \end{subfigure} \\
                              \begin{subfigure}[b]{0.49\linewidth}
                                    \centering
                                    \includegraphics[width=\linewidth]{figs/Q1_d_poly_3.pdf}
                                    \caption{$n = 3$}
                              \end{subfigure}
                              \begin{subfigure}[b]{0.49\linewidth}
                                    \centering
                                    \includegraphics[width=\linewidth]{figs/Q1_d_poly_9.pdf}
...
def normal_equation_method(xi: np.array, yi: np.array, n_poly: int, n_trig: int, tol: float = 1e-5):
    ...
    return sp.simplify(p), coeff
            ...
            \begin{figure}[H]
                  \centering
                  \includegraphics[width = .6\linewidth]{figs/Q2_guess.pdf}
                  \caption{$f(x) = 1.25 x - 0.75 + 0.625 \sin(5\pi x)$}
            \end{figure}
            The Method of Normal Equations:
            \begin{figure}[H]
                  \centering
                  \includegraphics[width = .6\linewidth]{figs/Q2_fit.pdf}
      ...
      return coeff
                        ...
                        \begin{figure}[H]
                              ...
                              \begin{subfigure}[b]{.49\linewidth}
                                    \includegraphics[width=.99\linewidth]{figs/Q4_clear_table_points.pdf}
                                    \caption{clear\_table.txt}
                              \end{subfigure}
                              \begin{subfigure}[b]{.49\linewidth}
                                    \includegraphics[width=.99\linewidth]{figs/Q4_clear_table_plane.pdf}
                  ...
                  \item Interested in your gecko robot, your cat jumps up on the table.
                        ...
                        Why?
                        \textbf{Well, clearly not all the points are close to the ideal plane.}
                        ...
                        \begin{figure}[H]
                              \centering
                              \begin{subfigure}[b]{.49\linewidth}
                                    \includegraphics[width=.99\linewidth]{figs/Q4_cluttered_table_plane.pdf}
                  ...
                  \item Can you suggest a way to still find a fit to the plane of the table regardless of clutter?
                        ...
                        In this context, the outlier is the point that is far from the fitted plane.

                        We can do the following steps:
                        \begin{enumerate}
                              \item List out all possible combinations of 3 non-colinear points
                              \item Fit a plane to each of these combinations
                              \item Calculate the number of points close to the fitted planes. Here, we need a distance threshold to determine "close"
                              \item Find the plane that's close to the most number of points
                              \item Return the inlier set, which is the 3 points and all the points "close" to its fitted plane
                        \end{enumerate}
                        Now there are several ways to improve this "deterministic" algorithm, to make it more "RANSAC" like:
                        \begin{itemize}
                              \item This algorithm is not "randomized" as we brute-force test on all possible planes, but we can set a hyperparameter $\lambda \in (0, 1]$ to randomly sample a percentage of these combinations.

                                    I found this formula to determine N:
                                    $$
                                          N = \frac{\log(1 - p)}{\log(1 - \omega^n)}
                                    $$
                                    Where $p$ is the probability of success, $\omega$ is the percentage of inliers, and $n$ is the number of data points required to estimate the model parameters (which is 3 in plane fitting).
                              \item The distance threshold is a hard criterion, and might require prior knowledge of the smoothness of the surface.
                                    Instead, we can use some adaptive soft criteria, e.g., statistical methods, evaluate the distribution of \# inliers (or percentage of inliers) vs this distance threshold, up to some granularity/quantization.
                                    \begin{itemize}
                                          \item e.g., if over 50\% of the points are within $0.01 \times \text{max distance}$, then this is probably a good plane, and all the points within this distance are probably inliers.
                                          \item Why is variance not a good criterion? Consider the tube-like point cloud in d) and e), points on the opposite plane will cause high variance
                                    \end{itemize}
                        \end{itemize}

                        So, formally, we define this criteria (See Fig.~\ref{fig: adaptive_inlier_criteria}):
                        \begin{enumerate}
                              \item Plot the inlier ratio curve, which is defined as the number of inliers vs the distance threshold, which ranges from 0 to the maximum distance, with approximately 100 discretization levels.
                              \item Cap the inlier ratio curve to only number of inliers greater than inlier ratio x total number of points
                              \item Differentiate twice the capped inlier ratio curve
                              \item Find the minimum of the second derivative, which indicates a sharp turn at knee (reflection) point, and indicates the desired soft distance threshold
                        \end{enumerate}
                        \begin{figure}[H]
                              \centering
                              \begin{subfigure}[b]{.49\linewidth}
                                    \includegraphics[width=.99\linewidth]{figs/Q4_cluttered_table_adaptive_inlier_criteria_inlier_ratio.pdf}
                                    \caption{Inlier Ratio Curve}
                              \end{subfigure}
                              \begin{subfigure}[b]{.49\linewidth}
                                    \includegraphics[width=.99\linewidth]{figs/Q4_cluttered_table_adaptive_inlier_criteria_normalized.pdf}
                                    \caption{Capped Normalized Inlier Ratio Curve}
                              \end{subfigure} \\
                              \centering
                              \begin{subfigure}[b]{.49\linewidth}
                                    \includegraphics[width=.99\linewidth]{figs/Q4_cluttered_table_adaptive_inlier_criteria_diff.pdf}
                                    \caption{$1^{\text{st}}$ Derivative of (b)}
                              \end{subfigure}
                              \begin{subfigure}[b]{.49\linewidth}
                                    \includegraphics[width=.99\linewidth]{figs/Q4_cluttered_table_adaptive_inlier_criteria_diff_diff.pdf}
                                    \caption{$2^{\text{nd}}$ Derivative of (b)}
                              \end{subfigure}
                              \caption{Statistical Analysis of Inlier Ratio Curve. Red line indicates the estimated inlier distance threshold}
                              \label{fig: adaptive_inlier_criteria}
                        \end{figure}
                        \begin{lstlisting}[language=Python]
def adaptive_inlier_criteria(
      points: np.ndarray,
      plane_coeffs: np.ndarray,
      inlier_ratio: float,
      plot=False,
      save_fig_location: str = None,
      statistical_quantization_level: int = 100,
      ):
      NUM_POINTS = points.shape[0]

      distance = point_to_plane_distance(points, plane_coeffs)
      x = np.linspace(0, distance.max(), statistical_quantization_level)
      y = np.array([np.sum(distance <= threshold) for threshold in x])

      inlier_min_count_index = np.where(y >= inlier_ratio * NUM_POINTS)[0][0]

      x_norm_cap = (x / x.max())[inlier_min_count_index:]
      y_norm_cap = (y / y.max())[inlier_min_count_index:]

      diff_curve = np.gradient(y_norm_cap, x_norm_cap)

      diff_2_curve = np.gradient(diff_curve, x_norm_cap)

      criteria_index = np.argmin(diff_2_curve) + inlier_min_count_index

      inlier_x_norm_threshold = x_norm_cap[np.argmin(diff_2_curve)]
      inlier_distance_threshold = x[criteria_index]
      inlier_count = y[criteria_index]

      if plot:
            print(f"inlier min # points: {y[inlier_min_count_index]}")

            # Plot the inlier ratio curve
            plt.plot(x[inlier_min_count_index:], y[inlier_min_count_index:])
            plt.axvline(inlier_distance_threshold, color="r", linestyle="--")
            plt.xlabel("Threshold")
            plt.ylabel("# Inliers")
            plt.title("Inlier Ratio Curve")
            plt.grid()

            if save_fig_location is not None:
                  plt.savefig(save_fig_location + "_inlier_ratio.pdf", transparent=True)

            plt.show()

            # Plot the normalized inlier ratio curve
            plt.plot(x_norm_cap, y_norm_cap)
            plt.axvline(inlier_x_norm_threshold, color="r", linestyle="--")
            plt.xlabel("Capped Normalized Threshold")
            plt.ylabel("Capped Normalized # Inliers")
            plt.title("Capped Normalized Inlier Ratio Curve")
            plt.grid()

            if save_fig_location is not None:
                  plt.savefig(save_fig_location + "_normalized.pdf", transparent=True)

            plt.show()

            # Plot the difference curve
            plt.plot(x_norm_cap, diff_curve)
            plt.axvline(inlier_x_norm_threshold, color="r", linestyle="--")
            plt.xlabel("Normalized Threshold")
            plt.ylabel("First Order Derivative")
            plt.title("First Order Derivative Curve")
            plt.grid()

            if save_fig_location is not None:
                  plt.savefig(save_fig_location + "_diff.pdf", transparent=True)

            plt.show()

            # Plot the second order difference curve
            plt.plot(x_norm_cap, diff_2_curve)
            plt.axvline(inlier_x_norm_threshold, color="r", linestyle="--")
            plt.xlabel("Normalized Threshold")
            plt.ylabel("Second Order Derivative")
            plt.title("Second Order Derivative Curve")
            plt.grid()

            if save_fig_location is not None:
                  plt.savefig(save_fig_location + "_diff_diff.pdf", transparent=True)

            plt.show()

            print(f"Inlier threshold: {inlier_distance_threshold}")
            print(f"# inliers: {inlier_count}")

      inlier_indices = np.where(distance <= inlier_distance_threshold)

      return inlier_distance_threshold, inlier_count, inlier_indices
                        \end{lstlisting}
                        \begin{lstlisting}[language=Python]
def RANSAC_fit_plane(
      ...
      sampled_combinations = draw_distinct_combinations(NUM_POINTS, SAMPLE_SIZE, N)

      # Init
      smallest_inlier_distance_threshold = np.inf
      best_plane_inlier_count = 0
      best_plane_indices = np.array([], dtype=int)
      best_plane_inlier_indices = np.array([], dtype=int)

      for random_index in sampled_combinations:
            coeff = fit_plane(points[random_index])
            # distance = point_to_plane_distance(points[random_index], coeff)
            # distance = np.sort(distance)

            inlier_distance_threshold, inlier_count, inlier_indices = (
                  adaptive_inlier_criteria(
                  points,
                  coeff,
                  inlier_ratio,
                  statistical_quantization_level=statistical_quantization_level,
                  )
            )

            if inlier_distance_threshold <= smallest_inlier_distance_threshold:
                  smallest_inlier_distance_threshold = inlier_distance_threshold
                  best_plane_inlier_count = inlier_count
                  best_plane_inlier_indices = inlier_indices
                  best_plane_indices = random_index

      print(f"Inliers ratio: {best_plane_inlier_count / NUM_POINTS}")
      print(f"Smallest inlier distance threshold: {smallest_inlier_distance_threshold}")
      return points[best_plane_inlier_indices]
                        \end{lstlisting}
                        Plane Equation:
                        \begin{align*}
                        ...
                        \begin{figure}[H]
                              \centering
                              \begin{subfigure}[b]{.49\linewidth}
                                    \includegraphics[width=.99\linewidth]{figs/Q4_cluttered_table_plane_ransac.pdf}
                  ...
                  \item Encouraged by your results when testing on a table, you move your geckobot into the hallway and take an observation saved as the provided clean\_hallway.txt.
                        ...
                        The original formulation of RANSAC is already capable of fitting planes softly, so we just need to remove inlier points of a fitted plane once at a time.
                        Plane Equations:
                        \begin{align*}
                              \begin{cases}
                        ...
                        \begin{figure}[H]
                              \centering
                              \begin{subfigure}[b]{.49\linewidth}
                                    \includegraphics[width=.99\linewidth]{figs/Q4_clean_hallway_plane_ransac_all.pdf}
                                    \caption{clean\_hallway.txt all plane overlapped}
                              \end{subfigure}
                              \begin{subfigure}[b]{.49\linewidth}
                                    \includegraphics[width=.99\linewidth]{figs/Q4_clean_hallway_extended_ransac_vis_all.pdf}
                                    \caption{visualization of all planes in open3d}
                              \end{subfigure}
                              \caption{extended RANSAC fitted plane on clean\_hallway.txt}
                        \end{figure}
                        \textbf{Run the Jupyter Notebook to visualize the plains interactively with open3d.}
                  ...
                  \item You decide it is time to test your gecko robot's suction feet and move it to a different hallway.
                        ...
                        Similarly, we just need to voxel\_downsample the non-uniform point cloud, then set the inlier\_ratio adaptively in each iteration.
                        \textbf{The criteria is the number of inlier points fitted to the planes.}
                        \begin{lstlisting}[language=Python]
def RANSAC_fit_multiple_planes(
    ...
    for i in range(num_planes)[::-1]:
        ...
        inlier_points = RANSAC_fit_plane(
            remaining_points,
            N=N,
            inlier_ratio=1 / (i + 1)
            - 0.1,  # - 0.1 to prevent setting inlier_ratio to 1
        )
        ...
        plot_point_and_plane(remaining_points)
    ...
    return planes
                        ...
                        \begin{figure}[H]
                              \centering
                              \begin{subfigure}[b]{.49\linewidth}
                                    \includegraphics[width=.99\linewidth]{figs/Q4_last_vis.png}
                                    \caption{Open3d visualization of 2 planes (other planes not visible in the same view direction)}
                              \end{subfigure}
                              \begin{subfigure}[b]{.49\linewidth}
                                    \includegraphics[width=.99\linewidth]{figs/Q4_cluttered_hallway_planes.pdf}
                        ...
                        \end{figure}
                        Here are the 2 best plane in terms of number of points:
                        \begin{align*}
                              \begin{cases}
